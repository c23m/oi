...
// (x1,y1) 左上角 (x2,y2) 右下角：s[x2][y2] - s[x1-1][y2] - s[x2][y1-1] + s[x1-1][y1-1]
```
### 3. 一维差分（区间加，单点查）
```cpp
vvi a(n+1);
vector<int> diff(n+2, 0); // 多开一个位置
diff[1] = a[1];
for (int i=2; i<=n; ++i) diff[i] = a[i] - a[i-1];
void add(int l, int r, int c) { d[l] += c; d[r+1] -= c; }
// 操作完后，前缀和得到原数组
```
...
## 离散化
```cpp
vector<int> v;                // 存放所有待离散化的值
sort(v.begin(), v.end());
v.erase(unique(v.begin(), v.end()), v.end());
// 查询原值 x 对应的下标（0‑based）
...
// DFS 预处理每个节点的深度和直接父亲
void dfs(int u, int fa, const vector<vi>& g, vi& dep, vector<vi>& f) {
    f[u][0] = fa;              // 向上跳 2^0 步 = 父亲
    dep[u] = dep[fa] + 1;      // 深度 = 父亲深度 + 1
    for (int v : g[u]) {
        if (v != fa) {
            dfs(v, u, g, dep, f);
        }
...
// LCA 查询
int lca(int u, int v, vi& dep, vector<vi>& f) {
...
// dep[i] : 节点 i 的深度（根深度为 1，根的父亲深度为 0）
vi dep(n + 1, 0);

// f[i][j] : 节点 i 向上跳 2^j 步到达的节点
vector<vi> f(n + 1, vector<int>(LOG, 0));
...
// 假设已预处理 dep, fa[][], LCA 函数
// 路径 u->v 上每个点权值 +1
int l = lca(u, v);
diff[u]++; diff[v]++; diff[l]--; diff[fa[l][0]]--;
// 最后 DFS 累加子节点到父节点
